...
class GLPSO_DE:
    def __init__(self, params) -> None:
        ...
        self.F = params['algorithm']['amplification_factor']
    ...
    def generate_vars(self):
        ...
        self.G = self.X[eval_values.index(self.best_eval)].copy() # global best
        ...
        for idx, x_arr in enumerate(self.X):
            ...
            for d in range(self.D):
                if random.random() < self.pm:
                    X_copy = self.X.copy()
                    X_copy.remove(x_arr)
                    x1, x2, x3, x4 = random.sample(X_copy, 4)
                    O[d] = x_arr[d] + self.F * (x1[d] - x2[d]) + self.F * (x3[d] - x4[d])
...
def main():
    ...
    glpso_de = GLPSO_DE(params)
    print(glpso_de.eval(glpso_de.G))
    glpso_de.do_epoch()
    print(glpso_de.eval(glpso_de.G))
